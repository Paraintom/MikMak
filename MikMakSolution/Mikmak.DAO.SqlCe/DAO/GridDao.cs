        ...
        private void InsertIntoDatabase(Tuple<int, Grid> toSave)
        {
            ...
            try
            {
                using (DbCommand cmd = MyConnection.CreateCommand())
                {
                    ...
                    if (ceCmd != null)
                    {
                        ceCmd.Parameters.Add(new SqlCeParameter("@BattleId", SqlDbType.Int));
                        ceCmd.Parameters.Add(new SqlCeParameter("@NumberLines", SqlDbType.SmallInt));
                        ceCmd.Parameters.Add(new SqlCeParameter("@NumberColumns", SqlDbType.SmallInt));
                        ceCmd.Parameters.Add(new SqlCeParameter("@IsGridShifted", SqlDbType.NChar));
                        ceCmd.Parameters.Add(new SqlCeParameter("@MoveNumber", SqlDbType.SmallInt));
                        ...
                        ceCmd.Parameters["@IsGridShifted"].Size = 5;
                        ceCmd.Parameters["@PawnLocations"].Size = 500;
                        ceCmd.Parameters["@CurrentMessage"].Size = 100;
                        ceCmd.Parameters["@LastMove"].Size = 100;
                        ceCmd.Prepare();

                        ceCmd.Parameters["@BattleId"].Value = toSave.Item1;
                        ceCmd.Parameters["@NumberLines"].Value = grid.NumberLines;
                        ceCmd.Parameters["@NumberColumns"].Value = grid.NumberColumns;
                        ceCmd.Parameters["@IsGridShifted"].Value = grid.IsGridShiftedToDb();
                        ceCmd.Parameters["@MoveNumber"].Value = grid.MoveNumber;
